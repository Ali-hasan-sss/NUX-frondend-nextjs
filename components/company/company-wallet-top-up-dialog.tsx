...
function TopUpPaymentForm({
  ...
  wallet: ReturnType<typeof companyWalletService>;
}) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${origin}${typeof window !== "undefined" ? window.location.pathname : ""}`,
        },
      ...
      if (error) {
        ...
        if (paymentIntentId) {
          try {
            const sync = await wallet.syncTopUpAfterPayment(paymentIntentId);
...
interface CompanyWalletTopUpDialogProps {
  companyId: string;
  open: boolean;
...
export function CompanyWalletTopUpDialog({
  companyId,
  open,
  onOpenChange,
  onCompleted,
}: CompanyWalletTopUpDialogProps) {
  ...
  const handleStart = async () => {
    ...
    setIntentError(null);
    try {
      const out = await wallet.createTopUpPaymentIntent(n);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn("max-w-md max-h-[90vh] overflow-y-auto")}>
        ...
        {!pk || !stripePromise ? (
          ...
          >
            <TopUpPaymentForm onSuccess={handleDone} paymentIntentId={paymentIntentId} wallet={wallet} />
          </Elements>
        )}
      </DialogContent>
    </Dialog>
